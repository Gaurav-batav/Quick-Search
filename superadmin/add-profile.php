...
include'includes/session.php';
	if(isset($_POST['add3'])){
		...
		//	$password = password_hash($password, PASSWORD_DEFAULT);
			...
			try{
					...
					$stmt = $conn->prepare("UPDATE tbl_profile SET status=:status WHERE profile_id=:profile_id");
			        $stmt->execute(['status'=>$status, 'profile_id'=>$profile_id]);
    				$stmt1 = $conn->prepare("INSERT INTO product_images(profile_id, original_path, small_path, medium_path, large_path) VALUES (:profile_id, :original_path, :small_path, :medium_path, :large_path)");
    				$stmt1->execute(['profile_id'=>$profile_id,'original_path'=>$originalPath.$imageName,'small_path'=>$smallPath.$imageName, 'medium_path'=>$mediumPath.$imageName, 'large_path'=>$largePath.$imageName]);
				    $_SESSION['success'] = 'Profile Create successfully';
    			} 
	...
	header('location: addprofile-finish.php');
?>