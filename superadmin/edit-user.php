...
?>
    ...
    <button type='button' class='btn-close btn-close-white' data-bs-dismiss='alert' aria-label='Close'></button>
                    ...
                    $id=intval($_GET['id']);
                    $query=mysqli_query($conn,"SELECT users.*,users.id as p_id,tbl_status.status as s_name,tbl_status.id as sid from users join tbl_status on tbl_status.status=users.status WHERE users.id='$id'");
